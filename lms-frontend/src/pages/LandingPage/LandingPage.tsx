...
interface FeautreProps{
    title : string;
    description : string;
    icon : string;
}
const feautres : FeautreProps[] = [
...
export const LandingPage = () => {
    return(
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-32">
          {
            feautres.map((feautre,index)=>(
                <Card key={index} className="hover:shadow-lg transition-shadow">
                   <CardHeader>
                    <div className="text-4xl mb-4">{feautre.icon}</div>
                    <CardTitle>{feautre.title}</CardTitle>
                   </CardHeader>
                   <CardContent>
                    <p className="text-muted-foreground">{feautre.description}</p>
                   </CardContent>
                </Card>
            ))
          }
